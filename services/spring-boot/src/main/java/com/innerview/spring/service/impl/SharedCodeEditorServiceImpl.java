...
public class SharedCodeEditorServiceImpl implements SharedCodeEditorService {
  ...
  public void init(String roomId) {
    if (activeSessions.get(roomId) != null) return;
  ...
  public void addUserToSession(UUID userId, String roomId) {
    ...
    log.info(
        "[Editor] User {} joined editor in room {}. Total viewers: {}",
        userId,
        roomId,
  ...
  public void removeUserFromSession(UUID userId, String roomId) {
    ...
    log.info(
        "[Editor] User {} left editor in room {}. Remaining viewers: {}",
        userId,
        roomId,
        activeUsers.size());
    ...
    if (activeUsers.isEmpty()) {
      log.info(
          "[Editor] Everyone left the editor in room {}. Final state will persist via debounce.",
          roomId);
    }
  }
  ...
  public void updateCode(String roomId, CodeUpdatePayload newCode) {
    ...
    long expectedMinVersion =
        roomVersions.computeIfAbsent(roomId, id -> new AtomicLong(0)).incrementAndGet();

    debounceWriter.schedule(
        roomId, newCode.getBase64Vector(), newCode.getPlainText(), expectedMinVersion);
  }

  @Override
  public CodeUpdatePayload getCodeSnapshot(String roomId) {
    log.info(
        "[Editor] Serving saved code from Redis for late joiner in room {} and i return code is{}",
        roomId,
        redisPersistenceService.getRoomState(roomId).get().plainText());
